...
def build_yr_scheduling_options(actor_opts, *args, **kwargs) -> InvokeOptions:
    ...
    concurrency_groups = kwargs.get("concurrency_groups", None)
    if max_concurrency is not None:
        ...
        if not isinstance(concurrency_groups, dict):
            raise TypeError("concurrency_groups must be a dict if provided")
...
class ActorClass:
    ...
    def options(self, *args, **kwargs):
        ...
        if not opt.get_if_exists:
            opt.get_if_exists = False if kwargs.get("get_if_exists") is None else kwargs.get("get_if_exists")
