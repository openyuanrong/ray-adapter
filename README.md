...
- 安装：`pip install https://openyuanrong.obs.cn-southwest-2.myhuaweicloud.com/ray_adapter-0.7.0-py3-none-any.whl`。
- 使用：将原来使用 Ray 开发的应用代码中的 `import ray` 替换为 `import ray_adapter as ray`，并关注接口的差异。
- 部署：应用及 openYuanrong 集群部署可查看 openYuanrong 文档的[安装部署](https://pages.openeuler.openatom.cn/openyuanrong/docs/zh-cn/latest/deploy/index.html)章节。
...
### available_resources_per_node 接口示例
...
### util.PlacementGroupSchedulingStrategy 接口示例
...
### util.NodeAffinitySchedulingStrategy 接口示例
...
### runtime_context().get_accelerator_ids 接口示例
...
### runtime_context().get_node_id 接口示例
...
### runtime_context().namespace 接口示例
