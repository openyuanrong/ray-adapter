...
SSLCertConfig GetSSLCertConfig(const CommonFlags &flags)
{
...
Status InitLitebusSSLEnv(const SSLCertConfig &sslCertConfig)
{
...
int Sha256CalculateFile(const char *file, unsigned char *sha256Value, int valueLength)
{
    FILE *fp;
    SHA256_CTX ctx;
...
int Sha512CalculateFile(const char *file, unsigned char *sha512Value, int valueLength)
{
    FILE *fp;
    SHA512_CTX ctx;
    const int size = BUFFER_SIZE_KB + 1;
