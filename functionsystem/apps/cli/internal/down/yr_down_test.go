...
func TestDownCmdMinimal(t *testing.T) {
	Convey("Test Down Command (Minimal Coverage)", t, func() {
		// 测试 InitCMD
		Convey("InitCMD", func() {
			Convey("Given a CmdIO instance", func() {
				...
				Convey("When InitCMD is called", func() {
					...
					Convey("Then it should set opts.cmdIO and return the command", func() {
						...
						So(cmd.Use, ShouldEqual, "down") // 只验证基本属性
					})
				})
			})
		})
		...
		// 测试 runDown
		Convey("runDown", func() {
			Convey("Given a valid config file path and Down succeeds", func() {
				cmd := &cobra.Command{}
				...
				Convey("When runDown is called", func() {
			...
			Convey("Given a config file path and Down fails", func() {
				cmd := &cobra.Command{}
				args := []string{"config.yaml"}
				config := &utils.ClusterConfig{}
				patches := gomonkey.NewPatches()
